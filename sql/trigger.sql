...
CREATE 
    ...
    DELIMITER $$
    CREATE 
    TRIGGER  tg_autoInsertToProductView
	AFTER INSERT ON product FOR EACH ROW 
    BEGIN 
    insert into v_product(sortName,ProductRootPrice,ProductSalePrice,ProductQuantityInStock,quantitySold,`Sort Description`,ProductRate)values(
	new.productName,New.ProductRootPrice,new.productSalePrice,new.productQuantityInStock,new.quantitySold,new.productDescription,new.productRate);
    END$$
	DELIMITER;
	DELIMITER $$
